...
if(localStorage.hasOwnProperty('LoungeStats_accounts')) accounts = JSON.parse(localStorage['LoungeStats_accounts']);
  ...
  var d = new Date(bits[2], bits[1]-1, bits[0]).getTime();
	...
	for(bet in bets) {
		...
		for(i in itemarray) {
			...
			if(cleanparse || (!cacheWeapons.hasOwnProperty(localKeyName) && !getItemPrice(itemname, date))) {
...
function generateStatsPage() {
	...
	for(var i in betsKeys) {
		...
		if(setting_bvalue == 1) {
			betData.push([setting_xaxis == 0 ? b['date'] : absoluteIndex, betValue, teamString]);
		}
		absoluteIndex++;
	}
...
function init() {
	...
	GM_addStyle(".jqplot-highlighter-tooltip {background-color: #393938; border: 1px solid gray; padding: 5px; color: #ccc} \
							 ...
							 #loungestats_settings_leftpanel select, #loungestats_settings_leftpanel input{margin: 5px 0; width: 100%; height: 22px !important; padding: 0;} \
